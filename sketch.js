...
function setup() {
  ...
  trex = createSprite(displayWidth-1200,displayHeight-40,20,50);
  ...
  gameover=createSprite(displayWidth/2,displayHeight/2,30,30);
...
function draw() {
  ...
  fill("black");
  textSize(20);
  text("Score : "+score,displayWidth/2 - 100,displayHeight+100);
  text("High Score : "+highscore,displayWidth/2 + 100,displayHeight+100);
  ...
  if(gameState===play){
     score=score+Math.round(getFrameRate()/60);
      ...
      if((touches.length>0 || keyDown("space")) && trex.y>=displayHeight-50) {
    trex.velocityY = -20;
...
if(frameCount % 80 ===0){
  cactus=createSprite(displayWidth,displayHeight-35,20,20);
  cactus.velocityX=-(5+score/300);
  cactus.scale=0.7;
  cactus.lifetime=displayWidth/-(5+score/300);
